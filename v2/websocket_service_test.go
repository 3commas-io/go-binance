...
func (s *websocketServiceTestSuite) TestPartialDepthServe() {
	...
	doneC, stopC, err := WsPartialDepthServe("ETHBTC", false, "5", func(event *WsPartialDepthEvent) {
...
func (s *websocketServiceTestSuite) TestPartialDepthServe100Ms() {
	...
	doneC, stopC, err := WsPartialDepthServe100Ms("ETHBTC", false, "5", func(event *WsPartialDepthEvent) {
		e := &WsPartialDepthEvent{
			Symbol:       "ETHBTC",
			LastUpdateID: 160,
			Bids: []Bid{
...
func (s *websocketServiceTestSuite) TestCombinedPartialDepthServe() {
	...
	s.mockWsServe(data, errors.New(fakeErrMsg))
	defer s.assertWsServe()
	doneC, stopC, err := WsCombinedPartialDepthServe(symbolLevels, false, func(event *WsPartialDepthEvent) {
		e := &WsPartialDepthEvent{
			Symbol:       "ETHUSDT",
			LastUpdateID: 160,
			Bids: []Bid{
...
func (s *websocketServiceTestSuite) TestDepthServe() {
	...
	doneC, stopC, err := WsDepthServe("ETHBTC", false, func(event *WsDepthEvent) {
		e := &WsDepthEvent{
			Event:         "depthUpdate",
			Time:          1499404630606,
			Symbol:        "ETHBTC",
...
func (s *websocketServiceTestSuite) TestDepthServe100Ms() {
	...
	doneC, stopC, err := WsDepthServe100Ms("ETHBTC", false, func(event *WsDepthEvent) {
		e := &WsDepthEvent{
			Event:         "depthUpdate",
			Time:          1499404630606,
			Symbol:        "ETHBTC",
...
func (s *websocketServiceTestSuite) TestCombinedDepthServe() {
	...
	s.mockWsServe(data, errors.New(fakeErrMsg))
	defer s.assertWsServe()
	doneC, stopC, err := WsCombinedDepthServe(symbols, false, func(event *WsDepthEvent) {
		e := &WsDepthEvent{
			Symbol:        "BTCUSDT",
			Time:          1629769560797,
...
func (s *websocketServiceTestSuite) TestCombinedDepthServe100Ms() {
	...
	s.mockWsServe(data, errors.New(fakeErrMsg))
	defer s.assertWsServe()
	doneC, stopC, err := WsCombinedDepthServe100Ms(symbols, false, func(event *WsDepthEvent) {
		e := &WsDepthEvent{
			Symbol:        "BTCUSDT",
			Time:          1629769560797,
			LastUpdateID:  13544037,
...
func (s *websocketServiceTestSuite) TestKlineServe() {
	...
	doneC, stopC, err := WsKlineServe("ETHBTC", false, "1m", func(event *WsKlineEvent) {
		e := &WsKlineEvent{
			Event:  "kline",
			Time:   1499404907056,
			Symbol: "ETHBTC",
...
func (s *websocketServiceTestSuite) TestWsAggTradeServe() {
	...
	doneC, stopC, err := WsAggTradeServe("ETHBTC", false, func(event *WsAggTradeEvent) {
		e := &WsAggTradeEvent{
			Event:                 "aggTrade",
			Time:                  1499405254326,
			Symbol:                "ETHBTC",
...
func (s *websocketServiceTestSuite) TestWsCombinedKlineServe() {
	...
	doneC, stopC, err := WsCombinedKlineServe(input, false, func(event *WsKlineEvent) {
		e := &WsKlineEvent{
			Event:  "kline",
			Time:   1499404907056,
			Symbol: "ETHBTC",
...
func (s *websocketServiceTestSuite) TestWsCombinedAggTradeServe() {
	...
	doneC, stopC, err := WsCombinedAggTradeServe([]string{"ETHBTC"}, false, func(event *WsAggTradeEvent) {
		e := &WsAggTradeEvent{
			Event:                 "aggTrade",
			Time:                  1499405254326,
			Symbol:                "ETHBTC",
...
func (s *websocketServiceTestSuite) testWsUserDataServe(data []byte, expectedEvent *WsUserDataEvent) {
	...
	doneC, stopC, err := WsUserDataServe("fakeListenKey", false, func(event *WsUserDataEvent) {
...
func (s *websocketServiceTestSuite) testWsCombinedUserDataServe(data []byte, expectedEvent *WsCombinedUserDataEvent) {
	...
	doneC, stopC, err := WsCombinedUserDataServe([]string{"fakeListenKey"}, false, func(event *WsCombinedUserDataEvent) {
...
func (s *websocketServiceTestSuite) TestWsMarketStatServe() {
	...
	doneC, stopC, err := WsMarketStatServe("BNBBTC", false, func(event *WsMarketStatEvent) {
		e := &WsMarketStatEvent{
			Event:              "24hrTicker",
			Time:               123456789,
			Symbol:             "BNBBTC",
...
func (s *websocketServiceTestSuite) TestWsCombinedMarketStatServe() {
	...
	doneC, stopC, err := WsCombinedMarketStatServe([]string{"BNBBTC"}, false, func(event *WsMarketStatEvent) {
		e := &WsMarketStatEvent{
			Event:              "24hrTicker",
			Time:               123456789,
			Symbol:             "BNBBTC",
...
func (s *websocketServiceTestSuite) TestWsAllMarketsStatServe() {
	...
	doneC, stopC, err := WsAllMarketsStatServe(false, func(event WsAllMarketsStatEvent) {
...
func (s *websocketServiceTestSuite) TestWsTradeServe() {
	...
	doneC, stopC, err := WsTradeServe("BNBBTC", false, func(event *WsTradeEvent) {
		e := &WsTradeEvent{
			Event:         "trade",
			Time:          123456789,
			Symbol:        "BNBBTC",
...
func (s *websocketServiceTestSuite) TestWsCombinedTradeServe() {
	...
	doneC, stopC, err := WsCombinedTradeServe([]string{"BNBBTC"}, false, func(event *WsCombinedTradeEvent) {
...
func (s *websocketServiceTestSuite) TestWsAllMiniMarketsStatServe() {
	...
	doneC, stopC, err := WsAllMiniMarketsStatServe(false, func(event WsAllMiniMarketsStatEvent) {
...
func (s *websocketServiceTestSuite) TestBookTickerServe() {
	...
	doneC, stopC, err := WsBookTickerServe("BTCUSD_200626", false, func(event *WsBookTickerEvent) {
		e := &WsBookTickerEvent{
			UpdateID:     17242169,
...
func (s *websocketServiceTestSuite) TestAllBookTickerServe() {
	...
	doneC, stopC, err := WsAllBookTickerServe(false, func(event *WsBookTickerEvent) {
		e := &WsBookTickerEvent{
			UpdateID:     17242169,
			Symbol:       "BTCUSD_200626",
			BestBidPrice: "9548.1",
